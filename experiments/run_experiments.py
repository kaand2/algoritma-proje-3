"""
Experiment driver for comparing SA and ACO on synthetic TSP instances.
...
4. Produces basic plots:
   ...
   - Approximation ratio (small instances with known optimum)
...
def ensure_instances() -> Dict[str, TSPInstance]:
    """
    Ensure that benchmark instances are available.

    If no instance files are found in ``data/instances``, they are generated.
    """
...
def run_all_experiments() -> None:
    ...
    # Experiment configuration (can be adjusted for deeper studies).
    ...
    # Write all rows to a single CSV for convenient analysis.
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(rows)

    # Create plots from the collected data.
...
def create_plots_from_rows(
    ...
    """
    Generate summary plots from the long-form results table.

    Parameters
    ----------
    rows:
        List of dictionaries, each describing one run.
    output_path:
        Path to save a PNG with multiple subplots.
    """
    # Organise data by (algorithm, instance).
    ...
    # For each (algorithm, instance), compute mean length/runtime and mean/min/max approx ratio.
    ...
    for (inst_name, alg), rlist in by_inst_alg.items():
        ...
        # Ignore NaNs when computing approximation ratio statistics.
    ...
    # Plot 1: Tour length vs number of cities.
    for alg in algs:
        xs = []
        ys = []
        for n in sizes:
    ...
    # Plot 2: Runtime vs number of cities.
    for alg in algs:
        xs = []
        ys = []
        for n in sizes:
    ...
    # Plot 3: Approximation ratio (small instances only) with error bars (min-max).
    for alg in algs:
        xs = []
        ys = []
        yerr_lower = []
        ...
        for s in summary:
            ...
            ys.append(mean_val)
            # Error bars: lower = mean - min, upper = max - mean
        ...
        if xs:
            # Use errorbar with asymmetric error bars to show min-max range
            axes[2].errorbar(
                xs,
                ys,
